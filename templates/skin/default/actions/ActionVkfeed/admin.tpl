...
{if !$bOk}
    Для работы плагина нам необходимо авторизоваться вконтакте (получить токен) от имени администратора группы/страницы:
    <br/>
...
{else}
    Токен успешно записан,
    <a href="{router page='vkfeed'}">вернуться</a>
    .
{/if}
...
{if !$bToken}
    ...
    <b>{cfg name='plugin.vkfeed.file'}</b>
    <br/>
...
{else}
    <br/>
    <br/>
    Токен записан в файле:
    <b>{cfg name='plugin.vkfeed.file'}</b>
    , не забывайте время от времени получать новый токен.
{/if}
